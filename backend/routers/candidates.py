...
def get_candidate_profile(candidate_id: int, db: Session = Depends(database.get_db), current_user: models.User = Depends(auth.get_current_user)):
    c = db.query(models.Candidate).filter(models.Candidate.id == candidate_id, models.Candidate.is_deleted == False).first()
    if not c:
        raise HTTPException(status_code=404, detail="Aday bulunamadı")
    match_scores = db.query(models.MatchScore).filter(models.MatchScore.candidate_id == candidate_id).all()
    return {
        "id": c.id,
        "name": c.name,
        "email": c.email,
        ...
        "areas_for_improvement": c.areas_for_improvement or [],
...
def get_candidate_timeline(candidate_id: int, db: Session = Depends(database.get_db), current_user: models.User = Depends(auth.get_current_user)):
    return get_candidate_activities(candidate_id, db)


@router.get("/{candidate_id}/notes", response_model=List[schemas.CandidateNoteOut])
def get_candidate_notes(candidate_id: int, db: Session = Depends(database.get_db), current_user: models.User = Depends(auth.get_current_user)):
    return db.query(models.CandidateNote).filter(models.CandidateNote.candidate_id == candidate_id).order_by(models.CandidateNote.created_at.desc()).all()


@router.post("/{candidate_id}/notes", response_model=schemas.CandidateNoteOut, status_code=201)
def create_candidate_note(
    candidate_id: int,
    note_in: schemas.CandidateNoteCreate,
    db: Session = Depends(database.get_db),
    current_user: models.User = Depends(auth.get_current_user)
):
    user_role_upper = current_user.role.upper() if current_user.role else ""
    if user_role_upper == "VIEWER":
        raise HTTPException(status_code=403, detail="İzleyici (Viewer) rolü aday notu ekleyemez.")
    ...
    note = models.CandidateNote(
        candidate_id=candidate_id,
        application_id=note_in.application_id,
        position_id=note_in.position_id,
...
def get_candidate_applications(candidate_id: int, db: Session = Depends(database.get_db), current_user: models.User = Depends(auth.get_current_user)):
    ...
    for app in apps:
        ...
        res.append({
            ...
            "position_title": pos.title if pos else "Silinmiş Pozisyon",
            "position_department": pos.department if pos else None,
            "status": app.status,
...
def get_candidate_reports(candidate_id: int, db: Session = Depends(database.get_db), current_user: models.User = Depends(auth.get_current_user)):
    apps = db.query(models.Application).filter(models.Application.candidate_id == candidate_id).all()
    position_ids = [app.position_id for app in apps]
    reports = db.query(models.PositionReport).filter(models.PositionReport.position_id.in_(position_ids)).all() if position_ids else []
    return [
        {
            "id": rep.id,
