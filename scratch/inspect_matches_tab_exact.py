...
for idx, line in enumerate(lines):
    if "v-if=\"workspaceTab === 'matches'\"" in line or 'v-if="workspaceTab === \'matches\'"' in line:
        start_idx = idx
        break

if start_idx != -1:
    print(f"Matches tab found starting at line {start_idx+1}")
    # Print next 80 lines
    for i in range(start_idx, min(len(lines), start_idx + 80)):
        sys.stdout.write(f"{i+1}: {lines[i]}")
else:
    print("Matches tab not found")
